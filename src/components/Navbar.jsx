...
const Navbar = () => {
  ...
  return (
    <Disclosure 
      as="nav"
      className="shadow-lg backdrop-filter backdrop-blur-lg bg-white py-2 bg-opacity-80 text-gray-400 sticky top-0 z-20 transition duration-300 px-4 sm:px-16 w-full flex items-center"
    >
      {({ open }) => (
        <>  
                ...
                <div className="flex space-x-8 items-center">
                  {navigation.map((item) => (
                    <Link
                      key={item.name}
                      to={item.href}
                      className={classNames(location.pathname === item.href ? "border-b-2 border-red-700 font-extrabold text-gray-800" : " hover:text-gray-600 font-semibold", "text-sm transition duration-100"
                      )}
